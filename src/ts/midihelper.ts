...
export class MidiHelper {
  ...
  private initDevices_(midiAccess: WebMidi.MIDIAccess) {
    ...
    for (let input = inputs.next(); input && !input.done; input = inputs.next()) {
      ...
      console.log('MidiHelper <- Input', input.value);
      input.value.addEventListener('midimessage', (event: WebMidi.MIDIMessageEvent) => {
        if (this.callback) {
    ...
    for (let output = outputs.next(); output && !output.done; output = outputs.next()) {
      console.log('MidiHelper -> Output', output.value);
  ...
  //       if (this.callback) {
  //         this.callback((midiAccess as any).data);
  //       }
  //     }, (err: Error) => console.log('Something went wrong', err));
  // }
